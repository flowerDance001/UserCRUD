...
//}}}
      ...
      */
			setValue: function(year, month, day)  {
				...
				for (var j = !begin ? -7 : 0, a, num; j < (!begin ? 35 : 42); j++) { 
					...
					if(self.disableDats) {
					    $.each(self.disableDats, function(i,item){
					...
					if(self.ableDats) {
					    $.each(self.ableDats, function(i,item){
						    if(date.getDate() === item.getDate() && date.getMonth() === item.getMonth()) {
							    a.removeClass(css.disabled);
							}
						});
					}
	...
	//}}}
	        disable: function(date){
			    if(!self.disableDats) self.disableDats = [];
			    self.disableDats.push(date);
			},
			able: function(date){
				if(!self.ableDats) self.ableDats = [];
			    self.ableDats.push(date);
			},
