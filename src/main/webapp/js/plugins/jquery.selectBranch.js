...
(function($){
    $.fn.selectBranch = function(options){
		...
		function init(){
			...
			provinceList.change(function(value){
				if (value){
					...
					handleCityList(value,bankSel.getValue?bankSel.getValue():bankSel.val());
				}
				else {
					reset();
				}
			...
			cityList.change(function(value){
				if (value){
					handleBranchList(provinceList.selectValue,value,bankSel.getValue?bankSel.getValue():bankSel.val());
				}
		...
		//handle city list
		function handleCityList(code,bankId,callback){
		    if(opts.areaInfo) {
			...
			$.ajax({
				...
				dataType: 'json',
				type:'POST',
				data: {regionId:code,bankId:bankId},
				success: function(res){
		...
		//handle Branch list
		function handleBranchList(pId,code, bankId,callback){
			$.ajax({
				url: options.branchUrl,
				dataType: 'json',
				type:'POST',
				data: {cityId:code,bankId:bankId,provinceId:pId},
				success: function(res){
