  ...
  $.fn.tooltip.defaults = {
    ...
    littlewindow.prototype = {
			...
			if(this.opts.modal) {
			    if(_this.over && _this.container.get(0)==$('body').get(0)) {
					...
					function setSize(){
						...
						if(_this.over) _this.over.css({'width':overW,'height':overH});
        ...
        close: function(){
			...
			if(this.opts.close) {
			   if(!this.isDistroy) this.opts.close.call(this);
			}
        },
        distroy: function(){
		    if(this.isDistroy) return;
			...
			if(this.contentParent && this.opts.contentRecovery && !this.opts.simple) {
				this.item.hide();
